...
public class CustomCaST extends AbstractST {
    ...
    void testReplacingCustomClusterKeyPairToInvokeRenewalProcess() {
        ...
        Secret clusterCaCertificateSecret = kubeClient().getSecret(testStorage.getNamespaceName(), KafkaResources.clusterCaCertificateSecretName(testStorage.getClusterName()));
        ...
        clusterCaCertificateSecret = kubeClient().getSecret(testStorage.getNamespaceName(), KafkaResources.clusterCaCertificateSecretName(testStorage.getClusterName()));
    ...
    void testCustomClusterCaAndClientsCaCertificates() {
        ...
        final X509Certificate clientsCert = SecretUtils.getCertificateFromSecret(kubeClient().getSecret(testStorage.getNamespaceName(),
            KafkaResources.clientsCaCertificateSecretName(testStorage.getClusterName())), "ca.crt");
        final X509Certificate clusterCert = SecretUtils.getCertificateFromSecret(kubeClient().getSecret(testStorage.getNamespaceName(),
        ...
        final X509Certificate kafkaCert = SecretUtils.getCertificateFromSecret(kubeClient().getSecret(testStorage.getNamespaceName(),
        ...
        if (!Environment.isKRaftModeEnabled()) {
            X509Certificate zookeeperCert = SecretUtils.getCertificateFromSecret(kubeClient().getSecret(testStorage.getNamespaceName(),
        ...
        final X509Certificate userCert = SecretUtils.getCertificateFromSecret(kubeClient().getSecret(testStorage.getNamespaceName(),
    ...
    void testReplaceCustomClientsCACertificateValidityToInvokeRenewalProcess() {
        ...
        X509Certificate userCert = SecretUtils.getCertificateFromSecret(kubeClient().getSecret(testStorage.getNamespaceName(), testStorage.getUsername()), "user.crt");
        ...
        userCert = SecretUtils.getCertificateFromSecret(kubeClient().getSecret(testStorage.getNamespaceName(), testStorage.getUsername()), "user.crt");
    ...
    private void manuallyRenewCa(TestStorage testStorage, SystemTestCertHolder oldCa, SystemTestCertHolder newCa) {
        ...
        Secret caCertificateSecret = kubeClient().getSecret(testStorage.getNamespaceName(), certSecretName);
        Secret caKeySecret = kubeClient().getSecret(testStorage.getNamespaceName(), keySecretName);
    ...
    private void prepareTestCaWithBundleAndKafkaCluster(final SystemTestCertHolder certificateAuthority,
        ...
        final X509Certificate certificate = SecretUtils.getCertificateFromSecret(kubeClient().getSecret(testStorage.getNamespaceName(),
